...
- **Rounds requested quantity** — rounds the customer's requested quantity up to a configurable multiple, then checks further multiples above that floor for one with a *better per-unit price* that still clears 100% acceptance (never a larger total at a worse per-unit price). The search is bounded by an exact price ceiling plus a hard safety cap, not by the first infeasible multiple it sees — acceptance isn't guaranteed to be monotonic in quantity. Set `roundingMultiple` to `0` to pass the quantity through unchanged.
